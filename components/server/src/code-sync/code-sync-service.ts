...
const theiaPluginsRev = 'theia-plugins';
interface ISyncData {
    version: number;
    machineId?: string;
    content: string;
    ...
    get apiRouter(): express.Router {
        ...
        router.get('/v1/manifest', async (req, res) => {
            ...
            if (!manifest.latest.extensions) {
                manifest.latest.extensions = theiaPluginsRev;
            }
            res.json(manifest);
            return;
        });
        ...
        router.get('/v1/resource/:resource/:ref', async (req, res) => {
            ...
            if (!(resourceKey == SyncResource.Extensions && resourceRev === theiaPluginsRev)) {
                resourceRev = (await this.db.getResource(req.user.id, resourceKey, resourceRev))?.rev;
            }
            if (!resourceRev && resourceKey === SyncResource.Extensions) {
                resourceRev = theiaPluginsRev;
            }
            if (!resourceRev) {
            ...
            let content: string;
            if (resourceKey === SyncResource.Extensions && resourceRev === theiaPluginsRev) {
                content = await this.theiaPluginService.getCodeSyncResource(req.user.id);
                content = JSON.stringify(<ISyncData>{ version: 1, content });
            } else {
        ...
        router.post('/v1/resource/:resource', bodyParser.text({
            ...
            if (resourceKey === SyncResource.Extensions && latestRev === theiaPluginsRev) {
                latestRev = undefined;
            }
